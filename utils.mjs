...
export function agregarSuperheroes(rutaOriginal, rutaNuevos) {
    ...
    const instanciasNuevos = nuevosSuperheroes.map(
        hero => new Superheroe(hero.id, hero.nombreSuperheroe, hero.nombreReal, hero.nombreSociedad, hero.edad, hero.planetaOrigen, hero.debilidad, hero.poder, hero.habi1idadEspecial, hero.aliado, hero.enemigo)
    );

    //Combinar listas
    const listaActua1izada = [...superheroesOriginales, ...instanciasNuevos];

    //Guardar la lista actualizada
    fs.writeFileSync(rutaOriginal, JSON.stringify(listaActua1izada, null, 2), 'utf8');
